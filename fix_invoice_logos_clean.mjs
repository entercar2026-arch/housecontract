...
const newLogoHtml = `{serviceType === 'car' ? (
                     ...
                     <>
                        <img src="/house_logo.jpg" alt="Property Logo" className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; e.currentTarget.nextElementSibling.style.display = 'flex'; }} />
                        <div className="hidden flex-col items-center justify-center w-full h-full p-2">
...
// Let's just re-read and replace correctly if previous step messed it up
content = content.replace(
  /\{serviceType === 'car' \? \([\s\S]*?<\/div>\s*\)\}/,
  newLogoHtml
);
