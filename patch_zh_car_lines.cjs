const fs = require('fs');
let lines = fs.readFileSync('src/translations.tsx', 'utf8').split('\n');
...
for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes('carTerms:') && startIndex === -1) {
    // Only target the first one, which belongs to zh since en was skipped (it's at the top, wait, en is top, so it might be second)
    // We know zh is lines 168-278
    if (i > 160 && i < 300) {
      startIndex = i;
    }
  }
  if (startIndex !== -1 && i > startIndex && lines[i] === '    ]') {
    endIndex = i;
    break;
  }
}
...
if (startIndex !== -1 && endIndex !== -1) {
  lines.splice(startIndex, endIndex - startIndex + 1, ...newZhCarTerms);
  fs.writeFileSync('src/translations.tsx', lines.join('\n'));
  console.log("Patched zh carTerms perfectly!");
} else {
