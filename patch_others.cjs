...
async function run() {
  ...
  const textToTranslate = `You are a translator. Translate the following English strings into Japanese (ja), Korean (ko), and Russian (ru). 
Return ONLY a valid JSON object where keys are the language codes (ja, ko, ru) and values are objects with these keys: carLeaseTitle, carOwner, carRenter, carOwnerDesc, carTerms (an array of 14 strings for the 14 respects).
...
2. "Party (B) agrees to rent the vehicle described above from Party (A) for {contract.carPurpose}. Party (B) shall not use the vehicle for any purpose other than {contract.carPurpose}. Any change to the purpose of this rental requires prior written agreement."
3. "This contract is valid for a period of {contract.durationMonths} months, from {contract.startDate} to {endDate}."
4. "This car is rented for {contract.rentAmount} USD/month."
5. "Party (B) shall pay a security deposit of {contract.depositAmount} USD to Party (A). This deposit shall cover any intentional or unintentional damage caused to the vehicle during the rental period. Party (A) shall refund the deposit to Party (B) upon termination of the contract, following a vehicle inspection and confirmation that no damage has occurred. In the event that Party (B) terminates the contract prematurely, the deposit shall be automatically forfeited to Party (A)."
6. "Party (B) shall pay the rental fee to Party (A) on the {contract.startDate} of each month."
...
9. "9.1 In the event that the vehicle subject to the rental is slightly damaged or seriously damaged due to negligent use or fault of Party (B), the repair shall be the responsibility of Party (B), except in the case of wear and tear due to normal use, in which case the repair shall be the responsibility of Party (A). 9.2 Party (B) shall be fully responsible for any damage to the vehicle such as (collision, overturning, falling into water, theft, fire or other accidents) by paying compensation as agreed upon at a price of {contract.carCompensation} of the vehicle price to Party (A). 9.3 In the event that Party (B) uses the vehicle with collisions, scratches or other damages, Party (A) must be notified without allowing Party (B) to conceal the information of taking the vehicle for repair without the consent of Party (A)."
...
12. "The rental car in this contract is allowed to be used by Party (B) in the area around {contract.carRentalArea}. In case Party (B) uses it for a wrong purpose or goes to other provinces, it must notify Party (A) for permission, and Party (B) must be fully responsible for all costs and damages without involving Party (A)."
...
`;
  ...
  fs.writeFileSync('translated_others.json', response.text);
  console.log("Translation others done");
}
