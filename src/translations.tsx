...
export function translateUtilityValue(val: string | undefined, lang: string): string {
  if (!val) return '';
  let str = val.trim();
  const lower = str.toLowerCase();

  if (lower === 'none' || lower === 'no') {
    if (lang === 'km') return 'គ្មាន';
    if (lang === 'zh') return '无';
    if (lang === 'ja') return 'なし';
    if (lang === 'ko') return '없음';
  ...
  if (lower === 'free') {
    if (lang === 'km') return 'ឥតគិតថ្លៃ';
    if (lang === 'zh') return '免费';
    if (lang === 'ja') return '無料';
    if (lang === 'ko') return '무료';
  ...
  if (lower.includes('/month') || lower.includes('/ month')) {
    let monthTrans = '/month';
    if (lang === 'km') monthTrans = '/ខែ';
    else if (lang === 'zh') monthTrans = '/月';
    else if (lang === 'ja') monthTrans = '/月';
    else if (lang === 'ko') monthTrans = '/월';
    else if (lang === 'ru') monthTrans = '/мес.';
    else if (lang === 'vi') monthTrans = '/tháng';
    else if (lang === 'fr') monthTrans = '/mois';

    str = str.replace(/\/ ?month/gi, monthTrans);
  }

  if (lower.includes('/person') || lower.includes('/ person')) {
    let personTrans = '/person';
    if (lang === 'km') personTrans = '/នាក់';
    else if (lang === 'zh') personTrans = '/人';
    else if (lang === 'ja') personTrans = '/人';
    else if (lang === 'ko') personTrans = '/명';
    else if (lang === 'ru') personTrans = '/чел.';
    else if (lang === 'vi') personTrans = '/người';
    else if (lang === 'fr') personTrans = '/personne';

    str = str.replace(/\/ ?person/gi, personTrans);
  }
  ...
  en: {
    ...
    terms: [
      ...
      },
      {
        ...
        content: (contract) => <ul className="list-disc pl-6 space-y-2"><li>Party (A) ensures utilities are working before move-in.</li><li>Utility fees:<div className="space-y-1 mt-2"><div>- Water: <span className="font-bold">{translateUtilityValue(contract.waterUtility, 'en') || '.....'}</span></div><div>- Electricity: <span className="font-bold">{translateUtilityValue(contract.electricityUtility, 'en') || '.....'}</span></div><div>- Cable TV: <span className="font-bold">{translateUtilityValue(contract.cableTvUtility, 'en') || '.....'}</span></div><div>- Internet: <span className="font-bold">{translateUtilityValue(contract.internetUtility, 'en') || '.....'}</span></div>{(contract.otherUtility1Name || contract.otherUtility1Price) && (<div>- {contract.otherUtility1Name || '.....'}: <span className="font-bold">{translateUtilityValue(contract.otherUtility1Price, 'en') || '.....'}</span></div>)}{(contract.otherUtility2Name || contract.otherUtility2Price) && (<div>- {contract.otherUtility2Name || '.....'}: <span className="font-bold">{translateUtilityValue(contract.otherUtility2Price, 'en') || '.....'}</span></div>)}</div></li></ul>
      },
      {
...
,
  ...
  zh: {
    ...
    terms: [
      ...
      {
        title: "第四条: 水电及其他服务",
        content: (contract) => <ul className="list-disc pl-6 space-y-2"><li>甲方(A)确保入住前设施正常。</li><li>费用:<div className="space-y-1 mt-2"><div>- 水费: <span className="font-bold">{translateUtilityValue(contract.waterUtility, 'zh') || '.....'}</span></div><div>- 电费: <span className="font-bold">{translateUtilityValue(contract.electricityUtility, 'zh') || '.....'}</span></div><div>- 有线电视: <span className="font-bold">{translateUtilityValue(contract.cableTvUtility, 'zh') || '.....'}</span></div><div>- 网费: <span className="font-bold">{translateUtilityValue(contract.internetUtility, 'zh') || '.....'}</span></div>{(contract.otherUtility1Name || contract.otherUtility1Price) && (<div>- {contract.otherUtility1Name || '.....'}: <span className="font-bold">{translateUtilityValue(contract.otherUtility1Price, 'zh') || '.....'}</span></div>)}{(contract.otherUtility2Name || contract.otherUtility2Price) && (<div>- {contract.otherUtility2Name || '.....'}: <span className="font-bold">{translateUtilityValue(contract.otherUtility2Price, 'zh') || '.....'}</span></div>)}</div></li></ul>
      },
      {
        title: "第四条: 其他条件",
...
,
  ...
  ja: {
    ...
    terms: [
      ...
      {
        title: "第4条: 公共料金等",
        content: (contract) => <ul className="list-disc pl-6 space-y-2"><li>甲(A)は入居前に設備が正常に動作することを保証する。</li><li>料金:<div className="space-y-1 mt-2"><div>- 水道: <span className="font-bold">{translateUtilityValue(contract.waterUtility, 'ja') || '.....'}</span></div><div>- 電気: <span className="font-bold">{translateUtilityValue(contract.electricityUtility, 'ja') || '.....'}</span></div><div>- ケーブルTV: <span className="font-bold">{translateUtilityValue(contract.cableTvUtility, 'ja') || '.....'}</span></div><div>- インターネット: <span className="font-bold">{translateUtilityValue(contract.internetUtility, 'ja') || '.....'}</span></div>{(contract.otherUtility1Name || contract.otherUtility1Price) && (<div>- {contract.otherUtility1Name || '.....'}: <span className="font-bold">{translateUtilityValue(contract.otherUtility1Price, 'ja') || '.....'}</span></div>)}{(contract.otherUtility2Name || contract.otherUtility2Price) && (<div>- {contract.otherUtility2Name || '.....'}: <span className="font-bold">{translateUtilityValue(contract.otherUtility2Price, 'ja') || '.....'}</span></div>)}</div></li></ul>
      },
      {
        title: "第4条: その他の条件",
  ...
  ko: {
    ...
    terms: [
      ...
      {
        title: "제4조: 공과금",
        content: (contract) => <ul className="list-disc pl-6 space-y-2"><li>갑(A)은 입주 전 시설이 정상 작동함을 보장한다.</li><li>요금:<div className="space-y-1 mt-2"><div>- 수도: <span className="font-bold">{translateUtilityValue(contract.waterUtility, 'ko') || '.....'}</span></div><div>- 전기: <span className="font-bold">{translateUtilityValue(contract.electricityUtility, 'ko') || '.....'}</span></div><div>- 케이블 TV: <span className="font-bold">{translateUtilityValue(contract.cableTvUtility, 'ko') || '.....'}</span></div><div>- 인터넷: <span className="font-bold">{translateUtilityValue(contract.internetUtility, 'ko') || '.....'}</span></div>{(contract.otherUtility1Name || contract.otherUtility1Price) && (<div>- {contract.otherUtility1Name || '.....'}: <span className="font-bold">{translateUtilityValue(contract.otherUtility1Price, 'ko') || '.....'}</span></div>)}{(contract.otherUtility2Name || contract.otherUtility2Price) && (<div>- {contract.otherUtility2Name || '.....'}: <span className="font-bold">{translateUtilityValue(contract.otherUtility2Price, 'ko') || '.....'}</span></div>)}</div></li></ul>
      },
      {
        title: "제4조: 기타 조건",
  ...
  ru: {
    ...
    terms: [
      ...
      },
      {
        ...
        content: (contract) => <ul className="list-disc pl-6 space-y-2"><li>Сторона (А) гарантирует исправность коммуникаций перед въездом.</li><li>Тарифы:<div className="space-y-1 mt-2"><div>- Вода: <span className="font-bold">{translateUtilityValue(contract.waterUtility, 'ru') || '.....'}</span></div><div>- Электричество: <span className="font-bold">{translateUtilityValue(contract.electricityUtility, 'ru') || '.....'}</span></div><div>- Кабельное ТВ: <span className="font-bold">{translateUtilityValue(contract.cableTvUtility, 'ru') || '.....'}</span></div><div>- Интернет: <span className="font-bold">{translateUtilityValue(contract.internetUtility, 'ru') || '.....'}</span></div>{(contract.otherUtility1Name || contract.otherUtility1Price) && (<div>- {contract.otherUtility1Name || '.....'}: <span className="font-bold">{translateUtilityValue(contract.otherUtility1Price, 'ru') || '.....'}</span></div>)}{(contract.otherUtility2Name || contract.otherUtility2Price) && (<div>- {contract.otherUtility2Name || '.....'}: <span className="font-bold">{translateUtilityValue(contract.otherUtility2Price, 'ru') || '.....'}</span></div>)}</div></li></ul>
      },
      {
